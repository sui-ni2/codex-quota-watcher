...
function printHelp() {
  console.log(`Codex Quota Watcher 0.1.0
...
async function main() {
  ...
  const check = async () => {
    ...
    try {
      ...
      const { event, nextState } = evaluateTransition(previousState, snapshot);
      await saveState(options.stateFile, nextState);

      if (options.json || options.once) console.log(JSON.stringify(snapshot, null, 2));
      if (event) {
